...
import { roleBadgeClass, roleLabel } from '@/utils/format'
      ...
      <SidebarMenu>
        <SidebarMenuItem>
          <DropdownMenu>
            ...
            >
              {/* Signed-in account at a glance: photo, name, post and role. */}
              <div className="-mx-1 -mt-1 flex items-center gap-3 px-3 pt-3 pb-2">
                ...
                <div className="min-w-0 flex-1 leading-tight">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Badge
                      className={cn('shrink-0 px-1.5 py-0 text-[0.65rem]', roleBadgeClass(user.role))}
                    >
                      {roleLabel(user.role)}
                    </Badge>
                  </div>
                  {user.position ? (
                    <p className="truncate text-xs text-muted-foreground">{user.position}</p>
                  ) : null}
              ...
              <DropdownMenuItem className="cursor-pointer" onSelect={() => handOff(onViewProfile)}>
                <UserIcon />
                View profile
              </DropdownMenuItem>
              <DropdownMenuItem className="cursor-pointer" onSelect={() => handOff(onEditProfile)}>
                <PencilIcon />
                Edit profile
              </DropdownMenuItem>
              <DropdownMenuItem
